...
/** Simple absolutely-positioned image */
...
/** Flex-centered container with rotated inner */
function R({
...
/** Centered text line */
function Text({ top, size, weight, children }: { top: number; size: number; weight: number; children: React.ReactNode }) {
  return (
    <p style={{
      position: 'absolute',
      left: 'calc(50% - 0.5px)',
...
// ── Tablet canvas — 810 × 596 (exact Figma: 1868:1575) ───────────────────────
...
function TabletCanvas() {
  return (
    <>
      ...
      {/* Small floral — horizontally centered */}
...
// ── Mobile canvas — 375 × 596 (exact Figma: 1852:600) ────────────────────────
...
function ScaledCanvas({ w, h, pad, children }: { w: number; h: number; pad: { h: number; b: number }; children: React.ReactNode }) {
  ...
  return (
    <div style={{ paddingLeft: pad.h, paddingRight: pad.h, paddingBottom: pad.b, background: 'var(--bg)' }}>
